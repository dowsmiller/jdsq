...
for (i in seq_along(a_corpus_w)) {
    ...
    for (j in unique(a_corpus_w[[i]][, 3])) {
        words[j] <- a_corpus_w[[i]][, 1][a_corpus_w[[i]][, 3] == j][1]
    }
    first_words[[i]] <- words
}

names(first_words) <- names(a_corpus_w)
...
for (i in seq_along(a_corpus_lw)) {
    ...
    for (j in unique(a_corpus_lw[[i]][, 5])) {
        lemmata[j] <- a_corpus_lw[[i]][, 2][a_corpus_lw[[i]][, 5] == j][1]
    }
    first_lemmata[[i]] <- lemmata
}

names(first_lemmata) <- names(a_corpus_lw)
