...
#remove line numbers from a_corpus_c and _lc and convert list items to vectors
...
for (i in seq_along(a_corpus_w)) {
    words <- character()
    for (j in unique(a_corpus_w[[i]][, 3])) {
        words[j] <- a_corpus_w[[i]][, 1][a_corpus_w[[i]][, 3] == j][length(a_corpus_w[[i]][, 1][a_corpus_w[[i]][, 3] == j])]
    }
    last_words[[i]] <- words
}

names(last_words) <- names(a_corpus_w)
...
for (i in seq_along(a_corpus_lw)) {
    lemmata <- character()
    for (j in unique(a_corpus_lw[[i]][, 5])) {
        lemmata[j] <- a_corpus_lw[[i]][, 2][a_corpus_lw[[i]][, 5] == j][length(a_corpus_lw[[i]][, 2][a_corpus_lw[[i]][, 5] == j])]
    }
    last_lemmata[[i]] <- lemmata
}

names(last_lemmata) <- names(a_corpus_lw)
...
all_lemmata <- vector()
for (i in seq_along(a_corpus_lw)) {
    all_lemmata <- c(all_lemmata, a_corpus_lw[[i]][, 2])
}
