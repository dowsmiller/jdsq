...
#remove line numbers from p0_corpus_lc and and convert list items to vectors
...
for (i in seq_along(p0_corpus_w)) {
    ...
    for (j in unique(p0_corpus_w[[i]][, 3])) {
        words[j] <- p0_corpus_w[[i]][, 1][p0_corpus_w[[i]][, 3] == j][1]
    }
    first_words[[i]] <- words
}

names(first_words) <- names(p0_corpus_w)
...
for (i in seq_along(p0_corpus_lw)) {
    ...
    for (j in unique(p0_corpus_lw[[i]][, 5])) {
        lemmata[j] <- p0_corpus_lw[[i]][, 2][p0_corpus_lw[[i]][, 5] == j][1]
    }
    first_lemmata[[i]] <- lemmata
}

names(first_lemmata) <- names(p0_corpus_lw)
