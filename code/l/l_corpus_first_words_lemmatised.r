...
for (i in seq_along(l_corpus_w)) {
    ...
    for (j in unique(l_corpus_w[[i]][, 3])) {
        words[j] <- l_corpus_w[[i]][, 1][l_corpus_w[[i]][, 3] == j][1]
    }
    first_words[[i]] <- words
}

names(first_words) <- names(l_corpus_w)
...
for (i in seq_along(l_corpus_lw)) {
    ...
    for (j in unique(l_corpus_lw[[i]][, 5])) {
        lemmata[j] <- l_corpus_lw[[i]][, 2][l_corpus_lw[[i]][, 5] == j][1]
    }
    first_lemmata[[i]] <- lemmata
}

names(first_lemmata) <- names(l_corpus_lw)
