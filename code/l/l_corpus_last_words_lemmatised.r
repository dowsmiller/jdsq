...
#remove line numbers from l_corpus_c and _lc and convert list items to vectors
...
for (i in seq_along(l_corpus_w)) {
    words <- character()
    for (j in unique(l_corpus_w[[i]][, 3])) {
        words[j] <- l_corpus_w[[i]][, 1][l_corpus_w[[i]][, 3] == j][length(l_corpus_w[[i]][, 1][l_corpus_w[[i]][, 3] == j])]
    }
    last_words[[i]] <- words
}

names(last_words) <- names(l_corpus_w)
...
for (i in seq_along(l_corpus_lw)) {
    lemmata <- character()
    for (j in unique(l_corpus_lw[[i]][, 5])) {
        lemmata[j] <- l_corpus_lw[[i]][, 2][l_corpus_lw[[i]][, 5] == j][length(l_corpus_lw[[i]][, 2][l_corpus_lw[[i]][, 5] == j])]
    }
    last_lemmata[[i]] <- lemmata
}

names(last_lemmata) <- names(l_corpus_lw)
...
all_lemmata <- vector()
for (i in seq_along(l_corpus_lw)) {
    all_lemmata <- c(all_lemmata, l_corpus_lw[[i]][, 2])
}
