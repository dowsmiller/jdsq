...
for (i in seq_along(r_corpus_w)) {
    ...
    for (j in unique(r_corpus_w[[i]][, 3])) {
        words[j] <- r_corpus_w[[i]][, 1][r_corpus_w[[i]][, 3] == j][length(r_corpus_w[[i]][, 1][r_corpus_w[[i]][, 3] == j])]
    }
    last_words[[i]] <- words
}

names(last_words) <- names(r_corpus_w)
...
for (i in seq_along(r_corpus_lw)) {
    ...
    for (j in unique(r_corpus_lw[[i]][, 5])) {
        lemmata[j] <- r_corpus_lw[[i]][, 2][r_corpus_lw[[i]][, 5] == j][length(r_corpus_lw[[i]][, 2][r_corpus_lw[[i]][, 5] == j])]
    }
    last_lemmata[[i]] <- lemmata
}

names(last_lemmata) <- names(r_corpus_lw)
...
t.test(count_stanzas_ment, count_lines_ment$rate, paired = FALSE)


#box and whisker plot comparing rates for jdsq and r_corpus
df_ment_combined_j_r <- data.frame(
    type = c(rep("j", nrow(count_stanzas_ment)), rep("r", nrow(count_lines_ment))),
    rate = c(count_stanzas_ment$rate, count_lines_ment$rate)
)

rownames(df_ment_combined_j_r) <- c(rownames(count_stanzas_ment), rownames(count_lines_ment))

palette <- c("#006493", "#dd5182")

ggplot(
    df_ment_combined_j_r,
    aes(
        x = factor(type, levels = c("j", "r")),
        y = rate
    )
) +
geom_boxplot(
...
scale_y_continuous(labels = scales::percent) +
scale_x_discrete(labels = c("J-corpus", "R-corpus")) +
xlab("Corpus") +
ylab("Rate of Use of 'Easy' Rhymes (%)") +
theme_minimal() +
theme(
...
ggsave(
    filename = file.path(wdir, "ment_boxplot_j_r.png"),
    device = 'png',
    dpi = 700,
    width = 10,
    height = 5
...
all_lemmata <- vector()
for (i in seq_along(r_corpus_lw)) {
    all_lemmata <- c(all_lemmata, r_corpus_lw[[i]][, 2])
}
