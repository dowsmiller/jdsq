...
#remove line numbers from f_corpus_c and _lc and convert list items to vectors

for (i in seq_along(f_corpus_lc)) {
    text_i <- vector()
    for (j in 1:nrow(f_corpus_lc[[i]])) {
        text_i[j] <- f_corpus_lc[[i]][j, 2]
    }
    f_corpus_lc[[i]] <- text_i
}
...
for (i in seq_along(f_corpus_w)) {
    words <- character()
    for (j in unique(f_corpus_w[[i]][, 3])) {
        words[j] <- f_corpus_w[[i]][, 1][f_corpus_w[[i]][, 3] == j][length(f_corpus_w[[i]][, 1][f_corpus_w[[i]][, 3] == j])]
    }
    last_words[[i]] <- words
}

names(last_words) <- names(f_corpus_w)
...
for (i in seq_along(f_corpus_lw)) {
    lemmata <- character()
    for (j in unique(f_corpus_lw[[i]][, 5])) {
        lemmata[j] <- f_corpus_lw[[i]][, 2][f_corpus_lw[[i]][, 5] == j][length(f_corpus_lw[[i]][, 2][f_corpus_lw[[i]][, 5] == j])]
    }
    last_lemmata[[i]] <- lemmata
}

names(last_lemmata) <- names(f_corpus_lw)
...
all_lemmata <- vector()
for (i in seq_along(f_corpus_lw)) {
    all_lemmata <- c(all_lemmata, f_corpus_lw[[i]][, 2])
}
