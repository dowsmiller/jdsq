...
#remove line numbers from f_corpus_lc and and convert list items to vectors
...
for (i in seq_along(f_corpus_w)) {
    ...
    for (j in unique(f_corpus_w[[i]][, 3])) {
        words[j] <- f_corpus_w[[i]][, 1][f_corpus_w[[i]][, 3] == j][1]
    }
    first_words[[i]] <- words
}

names(first_words) <- names(f_corpus_w)
...
for (i in seq_along(f_corpus_lw)) {
    ...
    for (j in unique(f_corpus_lw[[i]][, 5])) {
        lemmata[j] <- f_corpus_lw[[i]][, 2][f_corpus_lw[[i]][, 5] == j][1]
    }
    first_lemmata[[i]] <- lemmata
}

names(first_lemmata) <- names(f_corpus_lw)
