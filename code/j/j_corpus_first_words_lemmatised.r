...
for (i in seq_along(j_corpus_w)) {
    ...
    for (j in unique(j_corpus_w[[i]][, 3])) {
        words[j] <- j_corpus_w[[i]][, 1][j_corpus_w[[i]][, 3] == j][1]
    }
    first_words[[i]] <- words
}

names(first_words) <- names(j_corpus_w)
...
for (i in seq_along(j_corpus_lw)) {
    ...
    for (j in unique(j_corpus_lw[[i]][, 5])) {
        lemmata[j] <- j_corpus_lw[[i]][, 2][j_corpus_lw[[i]][, 5] == j][1]
    }
    first_lemmata[[i]] <- lemmata
}

names(first_lemmata) <- names(j_corpus_lw)
