...
#remove line numbers from j_corpus_c and _lc and convert list items to vectors
...
for (i in seq_along(j_corpus_w)) {
    words <- character()
    for (j in unique(j_corpus_w[[i]][, 3])) {
        words[j] <- j_corpus_w[[i]][, 1][j_corpus_w[[i]][, 3] == j][length(j_corpus_w[[i]][, 1][j_corpus_w[[i]][, 3] == j])]
    }
    last_words[[i]] <- words
}

names(last_words) <- names(j_corpus_w)
...
for (i in seq_along(j_corpus_lw)) {
    lemmata <- character()
    for (j in unique(j_corpus_lw[[i]][, 5])) {
        lemmata[j] <- j_corpus_lw[[i]][, 2][j_corpus_lw[[i]][, 5] == j][length(j_corpus_lw[[i]][, 2][j_corpus_lw[[i]][, 5] == j])]
    }
    last_lemmata[[i]] <- lemmata
}

names(last_lemmata) <- names(j_corpus_lw)
...
all_lemmata <- vector()
for (i in seq_along(j_corpus_lw)) {
    all_lemmata <- c(all_lemmata, j_corpus_lw[[i]][, 2])
}
